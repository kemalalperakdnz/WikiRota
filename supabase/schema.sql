-- VikiRota çevrim içi liderlik tablosu
...
create table if not exists public.leaderboard_entries (
  ...
  route_history jsonb not null default '[]'::jsonb
    check (jsonb_typeof(route_history) = 'array'),
  steps integer not null check (steps between 1 and 1000),
  time_ms integer not null check (time_ms between 100 and 86400000),
...
create index if not exists leaderboard_rank_idx
  on public.leaderboard_entries (steps, time_ms, created_at);

create index if not exists leaderboard_filter_idx
  on public.leaderboard_entries
  (category_id, difficulty_id, steps, time_ms);
...
drop policy if exists "Leaderboard herkes tarafından okunabilir"
  on public.leaderboard_entries;
create policy "Leaderboard herkes tarafından okunabilir"
  on public.leaderboard_entries
  for select
